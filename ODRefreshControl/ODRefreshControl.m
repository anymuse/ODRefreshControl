...
- (instancetype)initWithFrame:(CGRect)frame activityIndicatorView:(UIView *)activity {
    ...
    if (self) {
        ...
        [[[UIColor darkGrayColor] colorWithAlphaComponent:0.5] CGColor];
...
(UIActivityIndicatorViewStyle)activityIndicatorViewStyle {
    if ([_activity isKindOfClass:[UIActivityIndicatorView class]]) {
        [(UIActivityIndicatorView *)_activity
         setActivityIndicatorViewStyle:activityIndicatorViewStyle];
    }
}
...
- (void)setActivityIndicatorViewColor:(UIColor *)activityIndicatorViewColor {
    if ([_activity isKindOfClass:[UIActivityIndicatorView class]] &&
        [_activity respondsToSelector:@selector(setColor:)]) {
...
- (UIColor *)activityIndicatorViewColor {
    if ([_activity isKindOfClass:[UIActivityIndicatorView class]] &&
        [_activity respondsToSelector:@selector(color)]) {
...
- (void)layoutSubviews {
    if (_refreshing) {
        ...
        if (distance == 0) {
            topOrigin =
            CGPointMake(floor(self.bounds.size.width / 2), bottomOrigin.y);
        } else {
            topOrigin = CGPointMake(floor(self.bounds.size.width / 2),
        ...
        // Top semicircle
        CGPathAddArc(path, NULL, topOrigin.x, topOrigin.y, currentTopRadius, 0,
                     M_PI, YES);
        ...
        CGPointMake(lerp((topOrigin.x - currentTopRadius),
                    ...
                    lerp(topOrigin.y, bottomOrigin.y, 0.2));
        CGPoint leftDestination =
        CGPointMake(bottomOrigin.x - currentBottomRadius, bottomOrigin.y);
        
        CGPathAddCurveToPoint(path, NULL, leftCp1.x, leftCp1.y, leftCp2.x,
                              leftCp2.y, leftDestination.x, leftDestination.y);
        
        // Bottom semicircle
        CGPathAddArc(path, NULL, bottomOrigin.x, bottomOrigin.y,
                     currentBottomRadius, M_PI, 0, YES);
        ...
        CGPointMake(lerp((topOrigin.x + currentTopRadius),
                         (bottomOrigin.x + currentBottomRadius), 0.9),
                    lerp(topOrigin.y, bottomOrigin.y, 0.2));
        CGPoint rightDestination =
        CGPointMake(topOrigin.x + currentTopRadius, topOrigin.y);
        
        CGPathAddCurveToPoint(path, NULL, rightCp1.x, rightCp1.y, rightCp2.x,
                              rightCp2.y, rightDestination.x, rightDestination.y);
        ...
        CGFloat currentArrowRadius =
        lerp(kMinArrowRadius, kMaxArrowRadius, percentage);
        ...
        CGPathAddArc(arrowPath, NULL, topOrigin.x, topOrigin.y, arrowBigRadius, 0,
                     3 * M_PI_2, NO);
        ...
        CGPathAddLineToPoint(arrowPath, NULL, topOrigin.x,
                             topOrigin.y - arrowBigRadius + currentArrowSize);
        CGPathAddArc(arrowPath, NULL, topOrigin.x, topOrigin.y, arrowSmallRadius,
                     3 * M_PI_2, 0, YES);
        ...
        CGPathAddArc(highlightPath, NULL, topOrigin.x, topOrigin.y,
                     currentTopRadius, 0, M_PI, YES);
        CGPathAddArc(highlightPath, NULL, topOrigin.x, topOrigin.y + 1.25,
                     currentTopRadius, M_PI, 0, NO);
...
- (void)beginRefreshing:(BOOL)animated {
    if (!_refreshing) {
        if (animated) {
            ...
            CGPoint topOrigin = CGPointMake(floor(self.bounds.size.width / 2),
                                            kMaxTopPadding + kMaxTopRadius);
            CGPathAddArc(toPath, NULL, topOrigin.x, topOrigin.y, radius, 0, M_PI,
                         YES);
            CGPathAddCurveToPoint(toPath, NULL, topOrigin.x - radius, topOrigin.y,
                                  topOrigin.x - radius, topOrigin.y,
                                  topOrigin.x - radius, topOrigin.y);
            CGPathAddArc(toPath, NULL, topOrigin.x, topOrigin.y, radius, M_PI, 0,
                         YES);
            CGPathAddCurveToPoint(toPath, NULL, topOrigin.x + radius, topOrigin.y,
                                  topOrigin.x + radius, topOrigin.y,
                                  topOrigin.x + radius, topOrigin.y);
            ...
            CABasicAnimation *shadowPathMorph =
            ...
            CABasicAnimation *shapeAlphaAnimation =
...
- (instancetype)initInScrollView:(UIScrollView *)scrollView
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, scrollView.frame.size.width, 0)]) {
        ...
        _contentView =
        [[ODRefreshControlDefaultContentView alloc] initWithFrame:self.bounds
...
- (CGFloat)navigationBarInset {
    if ([UIViewController instancesRespondToSelector:@selector(topLayoutGuide)]) {
        ...
        if (viewController.automaticallyAdjustsScrollViewInsets &&
            ...
            // There's a known bug in iOS7 where calling topLayoutGuide breaks
            // UITableViewControllers, so we do some manual calculations
            CGFloat size =
            MIN([UIApplication sharedApplication].statusBarFrame.size.height,
                [UIApplication sharedApplication].statusBarFrame.size.width);
            if (viewController.navigationController) {
                size += CGRectIntersection(
                                           viewController.navigationController.view.bounds,
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (_refreshing) {
                    ...
                    if (!self.scrollView.dragging) {
                        if (!_didSetInset) {
                            ...
                            if ([self.scrollView isKindOfClass:[UITableView class]]) {
                                for (int i = 0;
                                     i < [(UITableView *)self.scrollView numberOfSections];
                                     ++i) {
                                    if ([(UITableView *)self.scrollView rectForHeaderInSection:i]
                                        .size.height) {
        ...
        if (offset > 0 && _lastOffset > offset && !self.scrollView.isTracking) {
            // If we are scrolling too fast, don't draw, and don't trigger unless the
            // scrollView bounced back
            _canRefresh = NO;
            dontDraw = YES;
        }
        if (dontDraw) {
...
- (void)beginRefreshing {
    ...
    self.scrollView.contentInset = UIEdgeInsetsMake(
                                                    _currentTopInset + self.originalContentInset.top,
                                                    self.originalContentInset.left, self.originalContentInset.bottom,
                                                    self.originalContentInset.right);
    _ignoreInset = NO;
